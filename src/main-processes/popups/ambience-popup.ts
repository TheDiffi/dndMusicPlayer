...
ipcMain.on(IpcS.ambienceRequest, (event: any, ambienceId: string) => {
	const win = createAmbienceWindow();

	win.webContents.once("dom-ready", () => {
		//configures popup window
		win.webContents.send(IpcR.playAmbience, ambienceId);
...
ipcMain.on(IpcS.ambienceClose, (event: any, ambienceId: string) => {
	if (ambienceId === "closeAll") {
		...
		//responds to caller
		event.sender.send(IpcR.ambienceClosed, "closeAll");
	} else if (ambienceWindows.has(ambienceId)) {
		...
		mainWin.webContents.send(IpcR.ambienceClosed, ambienceId);
	}
});
